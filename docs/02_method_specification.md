...
`StationAgent` 由 ClientApp 实现本地训练/验证；`MetricsAgent` 汇总 MAE、RMSE、更新范数与公平性；`RulePlanningAgent` 使用预注册阈值；`LLMPlanningAgent` 使用同一观测空间；`CoordinatorAgent` 由动态策略负责 Schema 校验、广播、记录和失败处理。
