...
$superlink = Start-Process flower-superlink -ArgumentList '--insecure','--fleet-api-type','grpc-rere','--fleet-api-address','127.0.0.1:9092','--control-api-address','127.0.0.1:9093' -PassThru -WindowStyle Hidden -RedirectStandardOutput (Join-Path $logs 'superlink.out.log') -RedirectStandardError (Join-Path $logs 'superlink.err.log')
Start-Sleep -Seconds 3

$nodes = @()
for ($i = 0; $i -lt 12; $i++) {
    $nodeConfig = "partition-id=$i num-partitions=12"
    $nodes += Start-Process flower-supernode -ArgumentList '--insecure','--superlink','127.0.0.1:9092','--node-config',$nodeConfig -PassThru -WindowStyle Hidden -RedirectStandardOutput (Join-Path $logs "supernode_$i.out.log") -RedirectStandardError (Join-Path $logs "supernode_$i.err.log")
}
...
try {
    & flwr run . local-12 --run-config $runConfig
