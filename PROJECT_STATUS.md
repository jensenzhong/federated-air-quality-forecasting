...
| M1 项目初始化 | 门禁通过 | `uv sync --extra dev --frozen` 通过；2026-08-18 当前全量验证为 125 项测试通过、1 项完整数据测试因 `BEIJING_AQ_DATA_DIR` 未配置跳过；ruff、mypy（53 个源码入口）和 `git diff --check` 通过；报告依赖已完整锁定 | 无 |
...
- 已加入 `aqfl/evaluation/continual.py`，按 benchmark 论文定义 AF/AP/AvgPerf，并提供固定长度任务矩阵的安全聚合 codec；`aqfl/data/continual_dataset.py` 已把冻结的 T0/base-test/T1--T11 时间窗接到客户端本地 cache 索引和 80/20 任务切分，ClientApp 已在显式 `continual-enabled` 请求下选择当前任务本地训练窗口，但尚未接入 12 站正式 continual 任务调度和任务矩阵 SecAgg 数组。
...
2. C1--C3 协同闭环已完成并通过定向测试；下一步实现 continual ClientApp 任务适配器，并做 12 站 1 轮 P1 同进程隐私 smoke，再扩到 3 轮确认真实 ClientApp 会消费上一轮 directive。该 smoke 不作为正式隐私证据；通过后才做 seed42 验证集 10 轮 P2。
